##Shawn Striker::ASU-Dr.Plaisier's Lab::2019##
#Context
#This portion of code was written to fit in a pipeline of other programs and pythons scripts and run in series in a Unix environment. This specific piece is designed to take known gene regulators and their gene targets from a json file and create a gene regulatory network model for a specific set of input genes. The two output files are an mdraw file which contains the network model structure and a csv which in this case contains the same information but in a different format. This model then gets used as an input to the next part of the pipeline.
...
with open(inputFileName,'r') as infile:
    ...
    while 1:
        ...
        if not split[0] in cluster2tf[cancerType]:
            cluster2tf[cancerType].append(split[0])#this is why no GBM and others. Not in reg output.
    if not 'GMB' in cluster2tf.keys():
        cluster2tf['GBM'] = []
...
#listTf = []
#with open ('causalAndMechanistic_network_CNA_CNVkit_8_13_2019.txt','r') as infile:
#    while 1:
...
#        inline = inline.split(' ')
#        if inline[1] == 'TF':
#            if inline[0] in entrez2symbol.keys():
#                listTf.append(entrez2symbol[inline[0]])
#print listTf
...
output = {'CHIR_curve':[]}
for tumortype in cluster2tf.keys():
    inputTfs[tumortype] = cluster2tf[tumortype]
#    inputTfs = cluster2tf[tumortype]
    if tumortype is 'GBM':
        print 'Setting GBM Inputs'
        inputTfs['GBM'] = ['430', '1052', '1053', '1385', '84699', '9586', '1871', '1874', '144455', '79733', '1960', '1997', '2002', '2004', '80712', '2114', '2115', '2120', '51513', '2551', '2623', '2624', '2625', '9421', '3232', '10320', '3659', '3662', '3670', '91464', '3726', '10661', '11278', '128209', '10365', '9314', '1316', '51176', '9935', '23269', '4602', '4774', '4790', '7025', '9480', '5468', '5914', '5916', '3516', '5971', '864', '6257', '4093', '6659', '6660', '6662', '25803', '347853', '30009', '9496', '6929', '6925', '8463', '7022', '29842', '10155', '6935', '132625', '23051', '85416', '7707', '7764', '23528', '201516']
#    inputTfs[tumortype] = cluster2tf[tumortype]
    # Load up exprssion data
#    inputTfs[tumortype] = listTf
    df = pd.read_csv('Reconstructed_exprs/'+tumortype+'_TCGA_1_exprs_entrez.csv', header=0, index_col=0)#ACC_TCGA_1_exprs_entrez
...
#    output = {'CHIR_curve':[]}
    tfCascade[tumortype] = []
    corTfCascade[tumortype] = []
    output[tumortype] = []
    #famExpType = args.family
    famExpType = 1
    for TFreg in inputTfs[tumortype]:
        motifs = []
        ...
        for motif in motifs:
            if motif in data:
                for geneTarg in data[motif]:
                    if geneTarg in inputTfs[tumortype]:
...
#                                    print 'in',TFreg, geneTarg, r1
                                    ...
                                    if r1[0]>0 and TFreg in symbol2entrez and geneTarg in symbol2entrez:
                                        output[tumortype].append(['gene',symbol2entrez[TFreg],'gene',symbol2entrez[geneTarg],'positive','A','A'])
                                    if r1[0]<0 and TFreg in symbol2entrez and geneTarg in symbol2entrez:
                                        output[tumortype].append(['gene',symbol2entrez[TFreg],'gene',symbol2entrez[geneTarg],'negative','A','A'])
...
biotapestry('biotapestry_CHIR_curve'+'ALL'+'.csv', output, inputTfs.keys())

mdraw('mdraw_ALL', output)
